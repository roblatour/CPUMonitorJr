...
const bool reportInFahrenheit = false;                     // if true will report in Farenheit, if false will report in Celcius
...
void showCPUandTempertature() {
    ...
    if (currentReadingHasChangedFlag) {
        ...
        for (int r = 0; r < lcdRows; r++) {
            lcd.setCursor(0, r); lcd.print(currentCPUReadings[r]);
        }
        ...
        if ((lcdColumns == 16) && (numberOfCPUs > 8)) {
            ...
            lcd.setCursor(cpuCol, 0); lcd.print("  ");
            ...
            lcd.setCursor(cpuCol, 0); lcd.print(sUsage);
            ...
            lcd.setCursor(tempCol, lcdRows - 1); lcd.print(currentTempReading);
...
void updateTemperature(byte WholeNumber, byte DecimalNumber) {
    ...
    // temperature is passed from the computer in Farenheit, so if the option to display in Celcius is choosen in the user settings the temperature value is converted here
    ...
    if (reportInFahrenheit) {
        ...
        double fahrenheit = double(WholeNumber * 100 + DecimalNumber) / double(100);
        double celcius = double(fahrenheit - 32) * double(5) / double(9);

        if (numberOfCPUs > 15) {
            int iCelcius = round(celcius);
            sprintf(formatbuffer, "%3d", iCelcius);
        }
        else {
            dtostrf(celcius, 5, 2, formatbuffer);
        }
...
AsyncWebSocket ws("/cpumonitorjr");
...
void onWsEvent(AsyncWebSocket* server, AsyncWebSocketClient* client, AwsEventType type, void* arg, uint8_t* data, size_t len) {
    ...
    if (type == WS_EVT_CONNECT) {
        ...
        case 1:
            ...
            // update the tempurature from passed data
            // whole part of the temperature value is in byte 1, decimal part is in byte 2
            updateTemperature(data[1], data[2]);

            // byte 3 contains the number of CPUs
            // byte 4 to byte 4 and beyond contain the individual percent busy CPU ratings for each cpu

            // load CPUValues into an array to pass to updateBarGraph
            numberOfCPUs = data[3];
            uint8_t CPUValues[numberOfCPUs];
            for (uint8_t i = 0; i < numberOfCPUs; i++)
                CPUValues[i] = data[i + 4];
